...
def Patientlog():
    if request.method == 'POST':
        ...
        cur.execute("select * from patient_reg where Name=%s and Password=%s",(name,password))
...
def Patientreg():
    if request.method=='POST':
        ...
        if password1 == password2:
            sql="select * from patient_reg where Name='%s' and Email='%s'"%(name,email)
            cur.execute(sql)
            data=cur.fetchall()
            db.commit()
            print('----',data)
...
def myreport():
    if request.method=="POST":
        ...
        sql = "select FIlename,AES_DECRYPT(FileData, 'sec_key') from reports where Patientid='%s'"%(keyvalue)
        # sql="select Filedata from reports where Patientid='%s'"%(keyvalue)
        data=pd.read_sql_query(sql,db)
        return render_template("myreport.html", x=2,cols=data.columns.values,rows=data.values.tolist())
    return render_template("myreport.html",x=1)
...
def patientreq():
    if request.method=='POST':
        ...
        sql="insert into patientreq (Name,Type,Age,symptoms,AppointmentDate,Time) values ('%s','%s','%s','%s','%s','%s')"%(Name,doc,Age,Symptoms,AppointmentDate,Time)
        cur.execute(sql)
        db.commit()
...
def patientreports():
    sql="select Id,Filename,Patientid from reports"
    data=pd.read_sql_query(sql,db)
    return render_template("patientreports.html",cols=data.columns.values,rows=data.values.tolist())

@app.route("/vreport/<x>")
def vreport(x=0):
    sql = "select Id,AES_DECRYPT(FileData, 'sec_key') from reports where Id='%s'"%(x)
    data=pd.read_sql_query(sql,db)
...
def acceptreq(x=0,y='',z=''):
    ...
    if data.empty:
        ...
        sql = "update patientreq set status='accepted' where status='pending' and Id='%s' and Name='%s'" % (x, y)
        cur.execute(sql)
        db.commit()
...
def mergereq(x='',y='',z=''):
    ...
    print(y)
    print(z)
    sql="select name,Type,Age from patientreq where status='accepted' and Name='%s'"%(session['username'])
    cur.execute(sql)
    da=cur.fetchall()
    db.commit()
    ...
    if request.method=='POST':
        ...
        if password == conpassword:
            ...
            for i in data:
                ...
                sql="insert into docreg(Name,Department,Age,Number,Email,Password) values('%s','%s','%s','%s','%s','%s')"%(name,dept,age,number,email,password)
                cur.execute(sql)
                db.commit()
...
def acceptdoc(x='',y=''):
    sql="update docreg set status='accepted' where status='pending' and Name='%s' and Email='%s'"%(x,y)
    cur.execute(sql)
    db.commit()
    sender_address = 'abhijeet.borade@gmail.com'
    sender_pass = 'yctpdhwggsazwjag'
...
def doctorlog():
    if request.method=='POST':
        ...
        sql="select * from docreg where status='accepted' and name='%s'"%(Docname)
        cur.execute(sql)
        data=cur.fetchall()
        db.commit()
        print(data)
        ...
        if data !=[]:
            ...
            if Docname in i[0][1] and passcode in i[0][-2]:
                ...
                return render_template("docrequest.html",msg=msg,name=Docname)
        else:
...
def patientreport():
    if request.method=="POST":
        keyvalue=request.form['keyvalue']
        sql = "select Id,FIlename,AES_DECRYPT(FileData, 'sec_key') from reports where Patientid='%s'"%(keyvalue)
        # sql="select Filedata from reports where Patientid='%s'"%(keyvalue)
        data=pd.read_sql_query(sql,db)
...
def viewpatient():
    sql="select * from connectdata where Type='%s' and status='pending'"%(session['dept'])
    cur.execute(sql)
    data=cur.fetchall()
    db.commit()
    print(data)
...
def patientaccess(a='',b=0):
    ...
    if data !=[]:
        Email=data[0][0]
        session['email']=Email
        sql="update connectdata set status='accept' where status='pending' and PatientName='%s'"%(a)
        cur.execute(sql)
        db.commit()
...
def uploadfile(email=''):
    ...
    if request.method=='POST':
        ...
        sql="insert into reports(FileName,FileData,patientEmail,Status,Patientid) values(%s,AES_ENCRYPT(%s,'sec_key'),%s,%s,%s)"
        val=(n,data,session['email'],status,PatientId)
        cur.execute(sql,val)
        db.commit()
...
def viewfiles():
    try:
        sql="select Id,FileName,Filedata,PatientEmail from reports where PatientEmail='%s' and status='accepted'"%(session['email'])
        data = pd.read_sql_query(sql, db)
        db.commit()
        return render_template('files.html',cols=data.columns.values,rows=data.values.tolist())
    except:
        return render_template('files.html',filenames='nofiles')


@app.route('/performs/<d>')
def performs(d=0):
    print(d)
    sql="update reports set status='updated' where Id='%s' and PatientEmail='%s'"%(d,session['email'])
    cur.execute(sql)
    db.commit()
...
def generatekey(c=0):
    ...
    print(x)
    print(c)
    sql="update reports set Key1='%s',status='done' where Id = '%s' and status='updated' "%(x,c)
    cur.execute(sql)
    db.commit()
...
def allrequests():
    try:
        sql="select Id,FileName,PatientEmail,Key1 from reports where status='done' and PatientEmail='%s'"%(session['email'])
        data=pd.read_sql_query(sql,db)
        db.commit()
        return render_template('all.html',cols=data.columns.values,rows=data.values.tolist())
    except:
        return render_template('all.html',user_name='nomessage')
...
def sentmail(e='',k=0,z=0):
    ...
    ss.sendmail(sender_address, receiver_address, text)
    ss.quit()
    sql="update reports set Status='complete' where Id='%s' and PatientEmail='%s'"%(z,session['email'])
    cur.execute(sql)
    db.commit()
...
def viewreport():
    try:
        sql="select * from reports where status='done' and PatientEmail='%s'"%(session['email'])
        data=pd.read_sql_query(sql,db)
        db.commit()
        if request.method=='POST':
            keyvalue=request.form['keycvalue']
            print(keyvalue)
            sql="select * from reports where status='complete' and PatientEmail='%s'"%(session['email'])
            cur.execute(sql)
            data=cur.fetchall()
            db.commit()
            print(keyvalue)
            print("=========")
            print(data[0][-2])
            if keyvalue in data[0][-2]:
                print("====")
                sql="select AES_DECRYPT(FileData, 'sec_key') from reports where PatientEmail='%s'" % (session['email'])
                cur.execute(sql)
                data=cur.fetchall()
                db.commit()
